...
export function getInteractionResponse(
  ...
  candidates.sort((a, b) => {
    const scoreA =
      (a.minRebelliousness ? 1 : 0) +
      (a.maxRebelliousness ? 1 : 0) +
      (a.minAmbition ? 1 : 0) +
      (a.isMinister ? 1 : 0) +
      (a.party ? 1 : 0) +
      (a.faction ? 1 : 0);
    const scoreB =
      (b.minRebelliousness ? 1 : 0) +
      (b.maxRebelliousness ? 1 : 0) +
      (b.minAmbition ? 1 : 0) +
      (b.isMinister ? 1 : 0) +
      (b.party ? 1 : 0) +
      (b.faction ? 1 : 0);
  ...
  const topScore =
    candidates.length > 0
      ? (candidates[0].minRebelliousness ? 1 : 0) +
        (candidates[0].maxRebelliousness ? 1 : 0) +
        (candidates[0].minAmbition ? 1 : 0) +
        (candidates[0].isMinister ? 1 : 0) +
        (candidates[0].party ? 1 : 0) +
        (candidates[0].faction ? 1 : 0)
      : 0;

  const topCandidates = candidates.filter((c) => {
    const score =
      (c.minRebelliousness ? 1 : 0) +
      (c.maxRebelliousness ? 1 : 0) +
      (c.minAmbition ? 1 : 0) +
      (c.isMinister ? 1 : 0) +
      (c.party ? 1 : 0) +
      (c.faction ? 1 : 0);
