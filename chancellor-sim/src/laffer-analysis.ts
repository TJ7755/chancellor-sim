...
function estimateTaxRevenueAtRate(taxType: LafferTaxType, trialRate: number, state: GameState): number {
  ...
  switch (taxType) {
    case 'incomeTaxBasic': {
      const base =
        baseIncome +
        (trialRate - 20) * 7 +
        (fiscal.incomeTaxHigherRate - 40) * 2 +
    ...
    case 'incomeTaxHigher': {
      const base =
        baseIncome +
        (fiscal.incomeTaxBasicRate - 20) * 7 +
        (trialRate - 40) * 2 +
        (fiscal.incomeTaxAdditionalRate - 45) * 0.2;
      const effectiveMarginalRate = trialRate + fiscal.nationalInsuranceRate;
    ...
    case 'vat': {
      ...
      const behaviouralLoss =
        excessRate > 0 ? (baseVAT + (trialRate - 20) * 7.5) * (Math.pow(1.02, excessRate) - 1) * taxAvoidanceScale : 0;
    ...
    case 'corporationTax': {
      ...
      const avoidanceLoss =
        excessRate > 0
          ? (baseCorp + (trialRate - 25) * 3.2) * (Math.pow(1.035, excessRate) - 1) * taxAvoidanceScale
          : 0;
