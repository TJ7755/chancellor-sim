...
const EVENT_TEMPLATES: EventTemplate[] = [
  // === INTERNATIONAL CRISES ===
  {
    ...
    generate: (state) => ({
      ...
      description:
        'Geopolitical tensions in the Middle East have caused oil prices to spike by 40% overnight. Petrol prices are surging and inflation expectations are rising sharply.',
      immediateImpact: {
        inflation: 0.8,
        gdpGrowth: -0.3,
        approvalRating: -3,
      ...
      responseOptions: [
        {
          ...
          description:
            'Reduce fuel duty by 10p per litre for six months to shield motorists from the worst of the price rise.',
          politicalCost: 10,
          fiscalCost: 4.5,
  ...
  },
  {
    ...
    generate: (state) => ({
      ...
      description:
        'A major European bank has collapsed, triggering fears of systemic financial crisis. UK banks are heavily exposed. Markets are in freefall.',
      immediateImpact: {
        gdpGrowth: -1.2,
        unemployment: 0.3,
        approvalRating: -5,
      ...
      responseOptions: [
        {
          ...
          description:
            'Guarantee all deposits and extend unlimited liquidity to UK banks. Costs unknown but could be vast.',
          politicalCost: 20,
          fiscalCost: 15,
  ...
  // === DOMESTIC SHOCKS ===
  {
    ...
    generate: (state) => ({
      ...
      description:
        'A harsh winter has pushed the NHS into complete crisis. A&E waiting times exceed 12 hours, operations are cancelled en masse, and two hospitals have declared major incidents.',
      immediateImpact: {
        approvalRating: -6,
        pmTrust: -5,
      },
      responseOptions: [
        {
          ...
          description:
            'Provide immediate emergency funding for winter pressures and temporary staff. Will increase deficit.',
          politicalCost: 5,
          fiscalCost: 2,
        ...
        },
        {
          ...
          description:
            'Bring in armed forces medical personnel to support overwhelmed hospitals. Low cost but controversial.',
          politicalCost: 10,
          fiscalCost: 0.1,
  ...
  },
  {
    ...
    generate: (state) => ({
      ...
      description:
        'Teachers, nurses, and rail workers have announced coordinated strike action over pay. Unions demand inflation-matching pay rises. The TUC warns of a "summer of discontent".',
      immediateImpact: {
        approvalRating: -4,
        gdpGrowth: -0.2,
      },
      responseOptions: [
        {
          ...
          description:
            'Grant above-inflation pay settlements to public sector workers. Expensive but ends strikes quickly.',
          politicalCost: 5,
          fiscalCost: 8,
  ...
  // === MARKET PANICS ===
  {
    ...
    generate: (state) => ({
      ...
      description:
        'Bond markets have lost faith in UK fiscal policy. Gilt yields are spiking, pension funds are in distress, and the pound is plummeting. The IMF has issued a rare public warning.',
      immediateImpact: {
        giltYieldBps: 120,
        sterlingPercent: -6,
        approvalRating: -8,
      ...
      responseOptions: [
        {
          ...
          description:
            'Announce £30bn of immediate tax rises and spending cuts. Markets will stabilise but recession likely.',
          politicalCost: 40,
          fiscalCost: -30,
  ...
  },
  {
    ...
    generate: (state) => ({
      ...
      description:
        'The pound has fallen to its lowest level against the dollar in 40 years. Currency traders believe the UK has lost control of inflation.',
      immediateImpact: {
        sterlingPercent: -4.5,
        inflation: 0.5,
        approvalRating: -5,
      ...
      responseOptions: [
        {
          ...
          description:
            'Publicly call for Bank of England to raise rates sharply. Controversial but may steady currency.',
          politicalCost: 15,
          fiscalCost: 0,
  ...
  // === NATURAL DISASTERS ===
  {
    ...
    generate: (state) => ({
      ...
      description:
        'Unprecedented rainfall has caused catastrophic flooding across northern England. Thousands evacuated, infrastructure damaged, insured losses estimated at £3bn.',
      immediateImpact: {
        gdpGrowth: -0.15,
        approvalRating: -2,
      },
      responseOptions: [
        {
          ...
          description:
            'Provide £1.5bn for immediate relief and reconstruction. Show government cares but adds to deficit.',
          politicalCost: 5,
          fiscalCost: 1.5,
          rebuildingMonths: 36,
        ...
        },
        {
          ...
          description:
            'Rely primarily on private insurance with minimal state intervention. Fiscally prudent but unpopular.',
          politicalCost: 15,
          fiscalCost: 0.1,
          rebuildingMonths: 12,
  ...
  // === SCANDALS ===
  {
    ...
    generate: (state) => ({
      ...
      description:
        'The Sunday Times has revealed that a Treasury minister held offshore investments that benefited from a tax loophole your department created. Opposition demanding resignation.',
      immediateImpact: {
        approvalRating: -5,
        pmTrust: -4,
        politicalCapital: -15,
      ...
      responseOptions: [
        {
          ...
          description:
            'Immediately sack the minister and distance yourself. Quick resolution but shows poor judgement.',
          politicalCost: 10,
          fiscalCost: 0,
  ...
  },
  {
    ...
    probability: 0.15,
    conditions: (state) =>
      (state.political?.backbenchSatisfaction ?? 70) < 40 || (state.political?.governmentApproval ?? 50) < 30,
    generate: (state) => ({
      type: 'political_crisis',
      severity: 'minor',
      title: 'Backbench Unrest',
      description:
        'Labour backbenchers are openly criticising Treasury policy in WhatsApp groups and at PLP meetings. Loyalty is wearing thin.',
      immediateImpact: {
        politicalCapital: -5,
        pmTrust: -2,
      },
...
function checkHeadlineConditions(conditions: HeadlineCondition, state: any): boolean {
  ...
  if (conditions.minUnemployment !== undefined && (economic?.unemploymentRate ?? 0) < conditions.minUnemployment)
    return false;
  if (conditions.maxUnemployment !== undefined && (economic?.unemploymentRate ?? 0) > conditions.maxUnemployment)
    return false;

  if (conditions.minApproval !== undefined && (political?.governmentApproval ?? 50) < conditions.minApproval)
    return false;
  if (conditions.maxApproval !== undefined && (political?.governmentApproval ?? 50) > conditions.maxApproval)
    return false;
...
export function generateHeadline(state: any, newspaper: NewspaperSource): { headline: string; subheading: string } {
  ...
  if ((state?.devolution?.localGov?.section114Notices || 0) >= 5) {
    return {
      ...
      subheading:
        'A growing number of councils have issued Section 114 notices, intensifying pressure on central government.',
    };
  }
  ...
  const validHeadlines = NEWSPAPER_HEADLINES.filter((entry: HeadlineEntry) =>
    checkHeadlineConditions(entry.conditions, state)
  );
...
const GENERIC_HEADLINE_POOL: Array<{ headline: string; subheading: string }> = [
  {
    headline: 'Treasury Faces Tough Autumn Trade-Offs',
    subheading: 'Ministers weigh tax, spending, and borrowing choices as economic indicators send mixed signals.',
  },
  {
    headline: 'Regional Growth Gap Back in Focus',
    subheading: 'Business groups call for targeted transport, skills, and planning reform to unlock local investment.',
  },
  {
    headline: 'Mortgage Pressure Keeps Households Cautious',
    subheading: 'Higher refinancing costs continue to squeeze disposable income despite steadier inflation readings.',
  },
  {
    headline: 'NHS Managers Warn on Winter Capacity',
    subheading: 'Trust leaders say staffing and social care bottlenecks remain critical ahead of colder months.',
  },
  {
    headline: 'Treasury Signals “No Easy Options” on Fiscal Room',
    subheading: 'Officials point to fragile headroom and persistent demand pressures in key public services.',
  },
  {
    headline: 'Retail Footfall Softens Outside Major Cities',
    subheading: 'High streets report subdued demand as consumers prioritise essentials and utility bills.',
  },
  {
    headline: 'Business Investment Plans Hold, but Confidence Patchy',
    subheading: 'Firms cite policy uncertainty and financing costs as barriers to larger capital commitments.',
  },
  {
    headline: 'Public Sector Pay Talks Enter Delicate Phase',
    subheading: 'Departments seek settlements that avoid disruption while limiting long-term fiscal spillovers.',
  },
  {
    headline: 'Infrastructure Pipeline Under Review',
    subheading: 'Treasury and departments reassess sequencing to protect delivery and value for money.',
  },
  {
    headline: 'Exporters Seek Sterling Stability',
    subheading: 'Manufacturers say predictable currency conditions matter as much as headline tax rates.',
  },
  {
    headline: 'Treasury Urged to Clarify Medium-Term Tax Path',
    subheading: 'Employers and households call for fewer policy reversals and clearer multi-year signalling.',
  },
  {
    headline: 'Court Delays and Prison Pressures Raise Justice Alarm',
    subheading: 'Sector leaders warn prolonged backlogs are creating knock-on costs across policing and probation.',
  },
  {
    headline: 'Consumer Confidence Edges Higher, Spending Still Tentative',
    subheading: 'Sentiment improves modestly but households remain wary of future bills and borrowing costs.',
  },
  {
    headline: 'Fiscal Debate Shifts from Size to Composition',
    subheading: 'Economists argue the quality of spending changes now matters more than aggregate totals alone.',
  },
  {
    headline: 'Skills Shortages Persist in Key Sectors',
    subheading: 'Employers report recruitment bottlenecks in health, logistics, construction, and digital services.',
  },
  {
    headline: 'Energy Policy Choices Reopen Cost-of-Living Debate',
    subheading: 'Analysts say household bill trajectories depend on both global prices and domestic tax settings.',
  },
  {
    headline: 'City Traders Watch OBR Signals Closely',
    subheading: 'Markets focus on credibility, implementation detail, and delivery risk rather than headline rhetoric.',
  },
  {
    headline: 'Small Firms Ask for Simpler Tax Administration',
    subheading: 'Owners say compliance complexity is becoming a larger burden than statutory rates alone.',
  },
  {
    headline: 'Local Councils Warn of Service Strain',
    subheading: 'Authorities report rising demand in social care, temporary accommodation, and safeguarding.',
  },
  {
    headline: 'Treasury Briefing Points to “Narrow Landing Zone”',
    subheading: 'Officials describe a constrained path balancing growth ambitions with fiscal discipline.',
  },
  {
    headline: 'Rail Reliability and Bus Provision Become Budget Battleground',
    subheading: 'Regional leaders push for stable multi-year settlements to improve commuter confidence.',
  },
  {
    headline: 'Debt Interest Bills Keep Pressure on Spending Plans',
    subheading: 'Even modest market moves are reshaping departmental assumptions for the year ahead.',
  },
  {
    headline: 'BoE-Treasury Coordination Under Scrutiny',
    subheading: 'Commentators debate how fiscal and monetary settings interact in a low-growth environment.',
  },
  {
    headline: 'MPC Holds Despite Inflation Noise',
    subheading: 'Threadneedle Street keeps policy unchanged while members signal different risk priorities.',
  },
  {
    headline: 'Markets Parse Split MPC Vote for Rate Signal',
    subheading: 'Investors debate whether divisions on the committee point to a near-term policy pivot.',
  },
  {
    headline: 'Unexpected Hold Leaves Traders Repricing the Path',
    subheading: 'Sterling and gilt futures adjust as consensus expectations are challenged.',
  },
  {
    headline: 'Emergency Cut Talk Emerges as Growth Wobbles',
    subheading: 'Analysts assess whether a sharper policy response may be required if demand weakens further.',
  },
  {
    headline: 'Rate Path Unclear as MPC Messaging Diverges',
    subheading: 'Officials stress optionality, keeping both tightening and easing risks in play.',
  },
  {
    headline: 'Policy Fatigue Sets In as Households Seek Predictability',
    subheading: 'Voters tell pollsters they want fewer surprises and clearer long-term direction from ministers.',
  },
];

function generateSecondaryPageHeadlines(
  state: any,
  primaryHeadline: string
): Array<{ headline: string; subheading: string }> {
  ...
  if ((state?.political?.governmentApproval ?? 50) < 32) {
    pool.unshift({
      ...
      subheading:
        'Senior MPs warn that delivery concerns are now feeding directly into parliamentary management risks.',
    });
  }
...
function generateOppositionQuote(state: any, newspaper: NewspaperSource): OppositionQuote {
  ...
  const template =
    OPPOSITION_QUOTE_TEMPLATES.find((t) => t.conditions(state)) ||
    OPPOSITION_QUOTE_TEMPLATES[OPPOSITION_QUOTE_TEMPLATES.length - 1];

  // Get quotes for this newspaper
  const newspaperKey = newspaper.name
    .toLowerCase()
    .replace('the ', '')
    .replace('daily ', '') as keyof typeof template.quotes;
  ...
  switch (event.type) {
    case 'natural_disaster':
      if (event.title === 'Severe Flooding') {
        paragraphs.push(
          `Unprecedented rainfall has caused catastrophic flooding across northern England. Thousands of residents have been evacuated from their homes, with emergency services conducting rescue operations across multiple counties. The Environment Agency warned that water levels in some areas remain at dangerous levels.`
        );
        paragraphs.push(
          `Communities from Manchester to Leeds have been devastated by the worst flooding in a generation. Hospitals and care homes have been evacuated, transport links severed, and critical infrastructure including power stations and water treatment plants damaged. Preliminary estimates suggest insured losses could exceed £3bn.`
        );
        paragraphs.push(
          `${newspaper.bias === 'left' ? "Criticism has mounted over the Environment Agency's funding cuts, with campaigners arguing that climate adaptation spending has been neglected. Local councils warned last year that flood defences were deteriorating." : 'Questions have been raised about the adequacy of drainage systems and flood defence maintenance. Property owners in affected areas complained they had received little warning of the severity of the risks.'}  The government has declared the affected regions disaster areas.`
        );
        paragraphs.push(
          `The Chancellor said the government would "provide whatever support is necessary" to help communities recover. Parliament is expected to recall early to discuss emergency relief measures. Reconstruction is expected to take years, with social services and housing support likely to remain strained for months.`
        );
      }
      break;
    ...
    case 'international_crisis':
      if (event.title === 'Oil Price Shock') {
        paragraphs.push(
          `Geopolitical tensions in the Middle East have triggered a sharp spike in global oil prices. Crude surged 40% overnight following military escalation, pushing wholesale petrol and diesel prices to their highest levels in two years.`
        );
        paragraphs.push(
          `Motorists faced chaos at filling stations as drivers rushed to top up tanks before prices rose further. Industry groups warned that transport costs would soar, raising the spectre of HGV shortages and higher food prices. Airlines issued guidance that ticket prices would likely need to increase.`
        );
        paragraphs.push(
          `${newspaper.bias === 'left' ? 'Consumer groups warned that lower-income households would bear the brunt of rising energy bills.' : 'The government was criticised for having no strategic petroleum reserves to buffer price volatility.'} Economists flagged the risk of stagflation – a deadly combination of stalled growth and rising inflation.`
        );
      } else if (event.title === 'Global Financial Contagion') {
        paragraphs.push(
          `A major European bank collapsed today, triggering the largest banking crisis since 2008. The bankruptcy sent shockwaves through global financial markets, wiping out shareholders and triggering emergency central bank operations.`
        );
        paragraphs.push(
          `UK banks and pension funds held significant exposure to the failed institution. Markets in free fall, with FTSE 100 stocks down 12% at one point and banking stocks particularly hard hit. Credit default swap spreads spiked, reflecting fears of systemic contagion across the sector.`
        );
        paragraphs.push(
          `The Bank of England announced emergency liquidity measures and was in constant contact with international regulators. The Treasury was preparing contingency plans should UK institutions require emergency support. One leading economist warned "We may be looking at another Great Financial Crisis scenario."'`
        );
      }
      break;
    ...
    case 'market_panic':
      if (event.title === 'Gilt Market Crisis') {
        paragraphs.push(
          `Bond markets have turned decisively against the UK, triggering the worst gilt sell-off in decades. Yields surged across the curve as investors sold gilts in panic selling, triggering mechanical selling from pension funds and insurance companies holding long-duration portfolios.`
        );
        paragraphs.push(
          `Ten-year gilt yields spiked above 5.5%, raising immediate concerns about mortgage costs and government borrowing. The pound plunged to its worst level in months. The IMF issued a rare public warning about UK fiscal sustainability, raising questions about the government's credibility with bond markets.`
        );
        paragraphs.push(
          `Credit rating agencies put the UK on negative outlook, warning further deterioration could trigger a ratings downgrade. Fund managers reported that UK gilts were no longer viewed as safe assets, with foreign central banks selling positions. The crisis threatened the government's ability to finance the deficit at reasonable cost.`
        );
      } else if (event.title === 'Sterling Crisis') {
        paragraphs.push(
          `The pound has collapsed to its weakest level against the dollar in 40 years, as currency traders lost confidence in UK economic management. Sterling fell 6% in a single session, raising concerns about imported inflation.`
        );
        paragraphs.push(
          `The Bank of England was criticised for losing control of inflation expectations. Emerging market central banks are reportedly divesting from pound-denominated assets. Foreign investors signalled they were demanding a significant risk premium to hold UK-exposed assets.`
        );
        paragraphs.push(
          `Business groups warned that the weak pound, whilst helping exporters, would raise costs for imported goods and commodities. Energy prices and food inflation were expected to spike further as sterling buying power weak abroad.`
        );
      }
      break;
    ...
    case 'scandal':
      if (event.title === 'Tax Affairs Scandal') {
        paragraphs.push(
          `A senior government minister has been ensnared in a major tax avoidance scandal, with investigations revealing complex offshore structures minimising personal tax bills. The revelation came just weeks after the Chancellor demanded "everyone must pay their fair share."'`
        );
        paragraphs.push(
          `Opposition parties have called for the minister's resignation. Internal government sources suggested the matter was causing acute embarrassment. The revelation threatens to undermine the government's credibility on tax fairness at a politically sensitive moment.`
        );
        paragraphs.push(
          `This is the third such scandal in as many years, prompting questions about the government's ethical standards. ${newspaper.bias === 'left' ? 'Anti-corruption groups called for immediate reform of parliamentary standards.' : 'Supporters argued private financial affairs should not be scrutinised, but media maintained intense focus.'}`
        );
      }
      break;
    ...
    default:
      ...
      if (event.severity === 'crisis') {
        paragraphs.push(
          `Emergency measures are being considered, with the government convening crisis meetings. Senior civil servants are working around the clock to assess the full magnitude of the situation.`
        );
      }
  }
...
function generateSecondaryStory(state: any, newspaper: NewspaperSource, event: RandomEvent): string[] {
  ...
  if (event.type === 'natural_disaster' && emergencyProgrammes.length > 0) {
    ...
    if (recentProgrammes.length > 0) {
      ...
      paragraphs.push(
        `The Chancellor announced a £${prog.immediateCost_bn.toFixed(1)}bn emergency relief package following the disaster. ${prog.rebuildingMonths ? `Infrastructure rebuilding is expected to cost approximately £${prog.rebuildingCostPerMonth_bn.toFixed(1)}bn per month over the next ${prog.rebuildingMonths} months.` : ''} ${newspaper.bias === 'left' ? 'Critics argued the package was insufficient given the scale of the disaster.' : 'The scale of the government response drew mixed reactions from business groups.'}`
      );
    }
  }
  ...
  if (deficit > 5) {
    ...
    const emergencyContext =
      totalEmergencySpending > 0
        ? ` Emergency relief spending is adding £${totalEmergencySpending.toFixed(1)}bn per month to the deficit.`
        : '';
    paragraphs.push(
      `The fiscal position adds urgency to the crisis response. The budget deficit has reached ${deficit.toFixed(1)}% of GDP, with the Treasury borrowing £${((deficit * gdpNominal) / 100 / 12).toFixed(1)}bn per month. ${event.type === 'natural_disaster' ? 'Emergency relief spending will increase the deficit further.' : 'The government has limited fiscal headroom to respond to any economic shock.'}${emergencyContext} Public debt stands at ${debt.toFixed(0)}% of national income.`
    );
  }
  ...
  while (paragraphs.length === 0) {
    paragraphs.push(
      `The government must now decide how to respond whilst managing broader economic and political pressures.`
    );
  }
 ...
 */
function generateArticleParagraphs(
  state: any,
  newspaper: NewspaperSource,
  headline: string,
  event?: RandomEvent
  ...
  if (gdpGrowth < -0.1) {
    paragraphs.push(
      `The UK economy contracted by ${Math.abs(gdpGrowth).toFixed(1)}% last quarter, marking ${gdpGrowth < -0.2 ? 'a significant recession' : 'a technical recession'} as output fell for the second consecutive quarter. The Office for National Statistics data showed weakness across manufacturing, construction, and consumer-facing services.`
    );
    ...
    paragraphs.push(
      `The economy grew modestly at ${gdpGrowth.toFixed(1)}% last quarter, a subdued performance reflecting ongoing headwinds from tight monetary policy and weak business confidence. The Chancellor had forecast stronger growth at this stage of the recovery.`
    );
  }
  ...
  if (inflation > 3) {
    ...
    paragraphs.push(
      `Inflation remains ${inflation > 5 ? 'stubbornly' : 'persistently'} above the Bank of England's 2% target at ${inflation.toFixed(1)}%, ${newspaper.bias === 'left' ? 'hammering household budgets' : 'requiring continued monetary tightness'}. Real wages have ${realWageGrowth > 0 ? 'finally turned positive' : 'fallen ' + Math.abs(realWageGrowth).toFixed(1) + '%'}, adding to the cost-of-living pressures facing families.`
    );
  }
  ...
  if (unemployment > 5 || Math.abs(unemployment - 4) > 1) {
    paragraphs.push(
      `The unemployment rate ${unemployment > 5 ? 'climbed to' : 'held at'} ${unemployment.toFixed(1)}%, ${unemployment > 6 ? 'with redundancies accelerating across multiple sectors' : 'suggesting the labour market is loosening'}. The Treasury insists its policies are working to boost employment, but ${newspaper.bias === 'left' ? 'critics argue austerity is destroying jobs' : 'businesses complain the tax burden makes hiring unaffordable'}.`
    );
  }
  ...
  if (state.political?.governmentApproval !== undefined && state.political.governmentApproval < 40) {
    ...
    paragraphs.push(
      `The Chancellor's approval rating has ${approval < 30 ? 'collapsed' : 'fallen'} to ${approval.toFixed(0)}%, ${newspaper.bias === 'left' ? leftText : rightText}. Labour backbenchers are ${(state.political?.backbenchSatisfaction ?? 70) < 40 ? 'in open revolt' : 'increasingly restive'}, with several reportedly demanding a change of direction.`
    );
  }
  ...
  if (state.markets?.giltYield10y !== undefined && state.markets.giltYield10y > 4.5) {
    ...
    paragraphs.push(
      `Bond markets reacted ${reactionTone}, with 10-year gilt yields ${movementVerb} to ${giltYield.toFixed(2)}%. ${tail} ${spreadTail}`
    );
  }
  ...
  while (paragraphs.length < 3) {
    paragraphs.push(
      `Treasury sources insist the government's economic plan remains on track despite short-term turbulence. ${newspaper.bias === 'left' ? 'However, the evidence suggests otherwise' : 'Critics will doubtless disagree'}.`
    );
  }
...
export function generateNewspaper(state: any, event?: RandomEvent): NewsArticle {
  ...
  if (event) {
    ...
    if (event.severity === 'crisis') {
      headline =
        selectedNewspaper.style === 'tabloid'
    ...
    >
      <div
        style={{
          ...
          maxHeight: '80vh',
          overflow: 'auto',
          border: event.severity === 'crisis' ? '3px solid var(--color-bad)' : '2px solid var(--color-warning)',
        }}
      >
        <div style={{ marginBottom: '20px' }}>
          <div
            style={{
              ...
              padding: '4px 12px',
              backgroundColor:
                event.severity === 'crisis' ? 'var(--color-bad)' : event.severity === 'major' ? 'var(--color-warning)' : 'var(--color-neutral)',
          ...
          <h2 style={{ margin: '8px 0 16px 0', fontSize: '24px', fontWeight: 'bold', color: 'var(--color-text-primary)' }}>
            {event.title}
          </h2>
          <p style={{ fontSize: '16px', color: 'var(--color-text-primary)', lineHeight: '1.6' }}>{event.description}</p>
        </div>
        ...
        {event.responseOptions && event.responseOptions.length > 0 && (
          <div style={{ marginTop: '24px' }}>
            ...
            {event.responseOptions.map((option, idx) => (
              ...
              >
                <div style={{ fontWeight: 'bold', fontSize: '15px', marginBottom: '6px', color: 'var(--color-text-primary)' }}>
                  {option.label}
                </div>
                ...
                <div style={{ fontSize: '13px', color: 'var(--color-text-muted)' }}>
                  ...
                  {option.fiscalCost
                    ? ` • Fiscal impact: £${Math.abs(option.fiscalCost)}bn ${option.fiscalCost > 0 ? 'cost' : 'saving'}`
                    : ''}
                </div>
              </button>
            ))}
...
export const Newspaper: React.FC<NewspaperProps> = ({ article, onClose }) => {
  ...
  return (
    ...
    >
      <div
        style={{
          backgroundColor: newspaper.name === 'Financial Times' ? '#FFF1E5' : 'var(--color-bg-elevated)',
          width: '100%',
          maxWidth: '800px',
          maxHeight: '90vh',
          overflow: 'auto',
          border: isSpecialEdition ? '4px solid var(--color-bad)' : '1px solid var(--color-border-strong)',
        }}
      >
        {/* Masthead */}
        <div
        ...
        {/* Article */}
        <div style={{ padding: '30px', backgroundColor: 'var(--color-bg-surface)' }}>
          {/* Headline */}
          <h1
            style={{
          ...
          {Array.isArray(secondaryPageHeadlines) && secondaryPageHeadlines.length > 0 && (
            <div style={{ marginTop: '28px', borderTop: '2px solid var(--color-border)', paddingTop: '18px' }}>
              ...
              {secondaryPageHeadlines.map((item, idx) => (
                <div
                  key={`${item.headline}_${idx}`}
                  style={{ marginBottom: '12px', paddingBottom: '10px', borderBottom: '1px solid var(--color-border)' }}
                >
                  <div style={{ fontSize: '18px', fontWeight: 700, color: 'var(--color-text-primary)', lineHeight: 1.25 }}>
                    {item.headline}
                  </div>
                  <div
                    style={{ fontSize: '14px', color: 'var(--color-text-muted)', marginTop: '4px', lineHeight: 1.45 }}
                  >
                    {item.subheading}
                  </div>
                </div>
...
export const EventLogPanel: React.FC<EventLogPanelProps> = ({ events }) => {
  return (
    ...
    >
      ...
      {events.length === 0 && (
        <p style={{ color: 'var(--color-text-muted)', fontSize: '14px', fontStyle: 'italic' }}>No recent events</p>
      )}
        ...
        <div
          ...
          style={{
            ...
            borderLeft: `3px solid ${
              entry.event.severity === 'crisis' ? 'var(--color-bad)' : entry.event.severity === 'major' ? 'var(--color-warning)' : 'var(--color-border-strong)'
            }`,
          }}
        >
